...
The Bronx bot executable includes prebuilt modules for server management.

### Welcome & Leave Messages (`.welcome` / `.leave`)
Configure greeting and departure alerts for server members.
- `.welcome setup` - Step-by-step interactive configuration wizard.
- `.welcome channel <#channel|none>` - Route greeting messages to a channel or disable them.
- `.welcome role <role>` - Set an autorole to automatically assign to new users on arrival.
- `.welcome type <text|embed>` - Toggle between standard text messages and rich embeds.
- `.welcome message <text...>` - Set raw message greeting text.
- `.welcome edit <key:val...>` - Inline edit embed attributes (keys: `title`, `description`, `color`, `image`).
- `.welcome json <raw_json>` - Raw import of complete welcome embed structure.
- `.welcome test` - Simulates a member join event by giving you the configured autorole and sending the formatted welcome message.
- Similar subcommands are available for `.leave` (excluding the `role` autorole command).

### Event Logging (`.log` / `.logging`)
Link server-wide events and bot moderation audit trails to tracking channels.
- `.log aio` - Automatically setup a category called **Logging** containing channels `#edits`, `#deletions`, `#joins-leaves`, and `#moderation`, routing all events automatically.
- `.log channel <event_type> <#channel|none>` - Start or stop routing an event type to a channel.
- `.log list` - Show current log channel links.
- `.log type <event_type> <text|embed>` - Set log layout type to raw markdown text or rich embeds.
- `.log message <event_type> <text...>` - Customize log message body text. Supports newlines and placeholders.
- `.log edit <event_type> <key:val...>` - Inline edit embed attributes (keys: `title`, `description`, `color`, `image`).
- `.log json <event_type> <raw_json>` - Raw import of complete embed structure.

#### Log Event Types & Placeholders
Every log type supports dedicated placeholders which are resolved dynamically when the event fires:
- **`delete` / `message_delete`**: Message Deletions
  - `{author}` - Mention of message author
  - `{username}` - Username of author
  - `{channel}` - Mention of deletion channel
  - `{content}` - Full deleted message content
- **`edit` / `message_edit`**: Message Edits
  - `{author}` - Mention of editor
  - `{username}` - Username of editor
  - `{channel}` - Mention of channel
  - `{before}` - Original content
  - `{after}` - Revised content
- **`join` / `member_join`**: New Member Arrivals
  - `{user}` - Member mention
  - `{username}` - Username
  - `{membercount}` - Total server member count
- **`leave` / `member_leave`**: Member Departures
  - `{user}` - Member mention
  - `{username}` - Username
  - `{membercount}` - Total server member count
- **`mod` / `moderation`**: Moderation Actions (Timeout, Ban, Kick)
  - `{action}` - Name of action performed
  - `{invokingUser}` / `{moderator}` - Mention of moderator
  - `{affectedUser}` / `{target}` - Mention of targeted member
  - `{reason}` - Specified reason
  - `{details}` - Duration details
- **`user` / `user_update`**: Profile Changes
  - `{user}` - User mention
  - `{oldUser}` - Previous username
